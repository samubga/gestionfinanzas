...
3. Genera nuevos valores aleatorios para `JWT_SECRET` (mínimo 32 caracteres) y `INVITE_CODES`. Nunca uses las claves locales ni las añadas a Git.
...
gcloud run deploy gestionfinanzas-api --image europe-west3-docker.pkg.dev/PROJECT_ID/gestionfinanzas/api:latest --region europe-west3 --allow-unauthenticated --set-env-vars NODE_ENV=production,APP_ORIGIN=https://PROJECT_ID.web.app,RUN_LEGACY_ACCOUNT_MIGRATION=false --set-secrets DATABASE_URL=gestionfinanzas-database-url:latest,JWT_SECRET=gestionfinanzas-jwt-secret:latest,INVITE_CODES=gestionfinanzas-invite-codes:latest
```
